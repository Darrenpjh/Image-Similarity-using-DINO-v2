...
def get_indexed_filenames(qdrant, collection):
    ...
    existing = set()
    offset = 0
    limit = 20000  # Increased limit for fewer calls

    iterations = (count + limit - 1) // limit
...
def main():
    ...
    with tqdm(img_files, desc="Embedding new images") as pbar:
        for idx, fname in enumerate(pbar):
            ...
            records.append(
                models.Record(
                    id=int(os.path.splitext(fname)[0], 36) if fname.isalnum() else idx,  # unique id or idx
                    vector=emb,
